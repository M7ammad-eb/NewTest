    ...
    "object" == typeof module && "object" == typeof module.exports ? module.exports = global.document ? factory(global, !0) : function (w) {
        ...
        ("undefined" != typeof window ? window : this, function (window, noGlobal) {
            ...
            function dataAttr(elem, key, data) {
                ...
                if (data === undefined && 1 === elem.nodeType)
                    if (name = "data-" + key.replace(rmultiDash, "-$1").toLowerCase(), "string" == typeof(data = elem.getAttribute(name))) {
                        try {
            ...
            function defaultDisplay(nodeName) {
                ...
                return display || ("none" !== (display = actualDisplay(nodeName, doc)) && display || ((doc = (iframe = (iframe || jQuery("<iframe frameborder='0' width='0' height='0'/>")).appendTo(doc.documentElement))[0].contentDocument).write(), doc.close(), display = actualDisplay(nodeName, doc), iframe.detach()), elemdisplay[nodeName] = display),
                display
            }
            ...
            jQuery.extend = jQuery.fn.extend = function (argument_0) {
                ...
                for ("boolean" == typeof target && (deep = target, target = arguments[i] || {}, i++), "object" == typeof target || jQuery.isFunction(target) || (target = {}), i === length && (target = this, i--); i < length; i++)
                    if (null != (options = arguments[i]))
                        for (name in options)
                            src = target[name], target !== (copy = options[name]) && (deep && copy && (jQuery.isPlainObject(copy) || (copyIsArray = jQuery.isArray(copy))) ? (copyIsArray ? (copyIsArray = !1, clone = src && jQuery.isArray(src) ? src : []) : clone = src && jQuery.isPlainObject(src) ? src : {}, target[name] = jQuery.extend(deep, clone, copy)) : copy !== undefined && (target[name] = copy));
                return target
            },
            jQuery.extend({
            ...
            function (window) {
                function Sizzle(selector, context, results, seed) {
                    ...
                    newContext,
                    newSelector;
                    if ((context ? context.ownerDocument || context : preferredDoc) !== document && setDocument(context), results = results || [], !selector || "string" != typeof selector)
                        return results;
                ...
                for (i in support = Sizzle.support = {}, isXML = Sizzle.isXML = function (elem) {
                    ...
                    return doc !== document && 9 === doc.nodeType && doc.documentElement ? (docElem = (document = doc).documentElement, documentIsHTML = !isXML(doc), parent && parent !== parent.top && (parent.addEventListener ? parent.addEventListener("unload", function () {
                                setDocument()
            ...
            (jQuery.fn.init = function (selector, context) {
                ...
                return (elem = document.getElementById(match[2])) && elem.parentNode && (this.length = 1, this[0] = elem),
            ...
            jQuery.Callbacks = function (options) {
                ...
                self = {
                    ...
                    fireWith: function (context, args) {
                        return !list || fired && !stack || (args = [context, (args = args || []).slice ? args.slice() : args], firing ? stack.push(args) : fire(args)),
                        this
                    },
                    fire: function () {
            ...
            Data.prototype = {
                ...
                remove: function (owner, key) {
                    ...
                    if (key === undefined)
                        ...
                        jQuery.isArray(key) ? name = key.concat(key.map(jQuery.camelCase)) : (camel = jQuery.camelCase(key), name = key in cache ? [key, camel] : (name = camel)in cache ? [name] : name.match(rnotwhite) || []),
                        i = name.length;
                        for (; i--; )
                            delete cache[name[i]]
                    }
                ...
                data: function (key, value) {
                    ...
                    if (this.length && (data = data_user.get(elem), 1 === elem.nodeType && !data_priv.get(elem, "hasDataAttrs"))) {
                        for (i = attrs.length; i--; )
                            attrs[i] && 0 === (name = attrs[i].name).indexOf("data-") && (name = jQuery.camelCase(name.slice(5)), dataAttr(elem, name, data[name]));
                ...
                trigger: function (event, data, elem, onlyHandlers) {
                    ...
                    if (cur = tmp = elem = elem || document, 3 !== elem.nodeType && 8 !== elem.nodeType && !rfocusMorph.test(type + jQuery.event.triggered) && (0 <= type.indexOf(".") && (type = (namespaces = type.split(".")).shift(), namespaces.sort()), ontype = type.indexOf(":") < 0 && "on" + type, (event = event[jQuery.expando] ? event : new jQuery.Event(type, "object" == typeof event && event)).isTrigger = onlyHandlers ? 2 : 3, event.namespace = namespaces.join("."), event.namespace_re = event.namespace ? new RegExp("(^|\\.)" + namespaces.join("\\.(?:.*\\.|)") + "(\\.|$)") : null, event.result = undefined, event.target || (event.target = elem), data = null == data ? [event] : jQuery.makeArray(data, [event]), special = jQuery.event.special[type] || {}, onlyHandlers || !special.trigger || !1 !== special.trigger.apply(elem, data))) {
                        ...
                        for (i = 0; (cur = eventPath[i++]) && !event.isPropagationStopped(); )
                            event.type = 1 < i ? bubbleType : special.bindType || type, (handle = (data_priv.get(cur, "events") || {})[event.type] && data_priv.get(cur, "handle")) && handle.apply(cur, data), (handle = ontype && cur[ontype]) && handle.apply && jQuery.acceptData(cur) && (event.result = handle.apply(cur, data), !1 === event.result && event.preventDefault());
                        return event.type = type,
                        onlyHandlers || event.isDefaultPrevented() || special._default && !1 !== special._default.apply(eventPath.pop(), data) || !jQuery.acceptData(elem) || ontype && jQuery.isFunction(elem[type]) && !jQuery.isWindow(elem) && ((tmp = elem[ontype]) && (elem[ontype] = null), elem[jQuery.event.triggered = type](), jQuery.event.triggered = undefined, tmp && (elem[ontype] = tmp)),
                        event.result
                    }
                },
                    ...
                    if ((args[0] = event).delegateTarget = this, !special.preDispatch || !1 !== special.preDispatch.call(this, event)) {
                        for (handlerQueue = jQuery.event.handlers.call(this, event, handlers), i = 0; (matched = handlerQueue[i++]) && !event.isPropagationStopped(); )
                            for (event.currentTarget = matched.elem, j = 0; (handleObj = matched.handlers[j++]) && !event.isImmediatePropagationStopped(); )
                                event.namespace_re && !event.namespace_re.test(handleObj.namespace) || (event.handleObj = handleObj, event.data = handleObj.data, (ret = ((jQuery.event.special[handleObj.origType] || {}).handle || handleObj.handler).apply(matched.elem, args)) !== undefined && !1 === (event.result = ret) && (event.preventDefault(), event.stopPropagation()));
                        ...
                        for (; cur !== this; cur = cur.parentNode || this)
                            if (!0 !== cur.disabled || "click" !== event.type) {
                                for (matches = [], i = 0; i < delegateCount; i++)
                                    matches[sel = (handleObj = handlers[i]).selector + " "] === undefined && (matches[sel] = handleObj.needsContext ? 0 <= jQuery(sel, this).index(cur) : jQuery.find(sel, this, null, [cur]).length), matches[sel] && matches.push(handleObj);
            ...
            jQuery.Event = function (src, props) {
                ...
                src && src.type ? (this.originalEvent = src, this.type = src.type, this.isDefaultPrevented = src.defaultPrevented || src.defaultPrevented === undefined && !1 === src.returnValue ? returnTrue : returnFalse) : this.type = src,
            ...
            jQuery.fn.extend({
                on: function (types, selector, data, fn, one) {
                    ...
                    if ("object" == typeof types) {
                        for (type in "string" != typeof selector && (data = data || selector, selector = undefined), types)
                            this.on(type, selector, data, types[type], one);
                        return this
                    }
                    if (null == data && null == fn ? (fn = selector, data = selector = undefined) : null == fn && ("string" == typeof selector ? (fn = data, data = undefined) : (fn = data, data = selector, selector = undefined)), !1 === fn)
                        fn = returnFalse;
                    else if (!fn)
                        return this;
            ...
            jQuery.extend({
                ...
                style: function (elem, name, value, extra) {
                    if (elem && 3 !== elem.nodeType && 8 !== elem.nodeType && elem.style) {
                        ...
                        "string" === (type = typeof value) && (ret = rrelNum.exec(value)) && (value = (ret[1] + 1) * ret[2] + parseFloat(jQuery.css(elem, name)), type = "number"),
                        null != value && value == value && ("number" !== type || jQuery.cssNumber[origName] || (value += "px"), support.clearCloneStyle || "" !== value || 0 !== name.indexOf("background") || (style[name] = "inherit"), hooks && "set" in hooks && (value = hooks.set(elem, value, extra)) === undefined || (style[name] = value))
                    }
                },
            ...
            jQuery.extend({
                ...
                prop: function (elem, name, value) {
                    ...
                    nType = elem.nodeType;
                    if (elem && 3 !== nType && 8 !== nType && 2 !== nType)
                        return (1 !== nType || !jQuery.isXMLDoc(elem)) && (name = jQuery.propFix[name] || name, hooks = jQuery.propHooks[name]), value !== undefined ? hooks && "set" in hooks && (ret = hooks.set(elem, value, name)) !== undefined ? ret : elem[name] = value : hooks && "get" in hooks && null !== (ret = hooks.get(elem, name)) ? ret : elem[name]
                },
                propHooks: {
                    tabIndex: {
                        get: function (elem) {
                ...
                toggleClass: function (value, stateVal) {
                    ...
                    return "boolean" == typeof stateVal && "string" === type ? stateVal ? this.addClass(value) : this.removeClass(value) : jQuery.isFunction(value) ? this.each(function (i) {
                        ...
                        if ("string" === type)
                            ...
                            type !== strundefined && "boolean" !== type || (this.className && data_priv.set(this, "__className__", this.className), this.className = this.className || !1 === value ? "" : data_priv.get(this, "__className__") || "")
                    })
                },
            ...
            jQuery.extend({
                ...
                ajax: function (url, options) {
                    ...
                    if (deferred.promise(jqXHR).complete = completeDeferred.add, jqXHR.success = jqXHR.done, jqXHR.error = jqXHR.fail, s.url = ((url || s.url || ajaxLocation) + "").replace(rhash, "").replace(rprotocol, ajaxLocParts[1] + "//"), s.type = options.method || options.type || s.method || s.type, s.dataTypes = jQuery.trim(s.dataType || "*").toLowerCase().match(rnotwhite) || [""], null == s.crossDomain && (parts = rurl.exec(s.url.toLowerCase()), s.crossDomain = !(!parts || parts[1] === ajaxLocParts[1] && parts[2] === ajaxLocParts[2] && (parts[3] || ("http:" === parts[1] ? "80" : "443")) === (ajaxLocParts[3] || ("http:" === ajaxLocParts[1] ? "80" : "443")))), s.data && s.processData && "string" != typeof s.data && (s.data = jQuery.param(s.data, s.traditional)), inspectPrefiltersOrTransports(prefilters, s, options, jqXHR), 2 === state)
                        return jqXHR;
                    for (i in(fireGlobals = s.global) && 0 == jQuery.active++ && jQuery.event.trigger("ajaxStart"), s.type = s.type.toUpperCase(), s.hasContent = !rnoContent.test(s.type), cacheURL = s.url, s.hasContent || (s.data && (cacheURL = s.url += (rquery.test(cacheURL) ? "&" : "?") + s.data, delete s.data), !1 === s.cache && (s.url = rts.test(cacheURL) ? cacheURL.replace(rts, "$1_=" + nonce++) : cacheURL + (rquery.test(cacheURL) ? "&" : "?") + "_=" + nonce++)), s.ifModified && (jQuery.lastModified[cacheURL] && jqXHR.setRequestHeader("If-Modified-Since", jQuery.lastModified[cacheURL]), jQuery.etag[cacheURL] && jqXHR.setRequestHeader("If-None-Match", jQuery.etag[cacheURL])), (s.data && s.hasContent && !1 !== s.contentType || options.contentType) && jqXHR.setRequestHeader("Content-Type", s.contentType), jqXHR.setRequestHeader("Accept", s.dataTypes[0] && s.accepts[s.dataTypes[0]] ? s.accepts[s.dataTypes[0]] + ("*" !== s.dataTypes[0] ? ", " + allTypes + "; q=0.01" : "") : s.accepts["*"]), s.headers)
            ...
            jQuery.ajaxPrefilter("json jsonp", function (s, originalSettings, jqXHR) {
                ...
                if (jsonProp || "jsonp" === s.dataTypes[0])
                    return callbackName = s.jsonpCallback = jQuery.isFunction(s.jsonpCallback) ? s.jsonpCallback() : s.jsonpCallback, jsonProp ? s[jsonProp] = s[jsonProp].replace(rjsonp, "$1" + callbackName) : !1 !== s.jsonp && (s.url += (rquery.test(s.url) ? "&" : "?") + s.jsonp + "=" + callbackName), s.converters["script json"] = function () {
            ...
            jQuery.fn.load = function (url, params, callback) {
                ...
                self = this,
                off = url.indexOf(" ");
                return 0 <= off && (selector = jQuery.trim(url.slice(off)), url = url.slice(0, off)),
            ...
            jQuery.offset = {
                setOffset: function (elem, options, i) {
                    ...
                    ("absolute" === position || "fixed" === position) && -1 < (curCSSTop + curCSSLeft).indexOf("auto") ? (curTop = (curPosition = curElem.position()).top, curLeft = curPosition.left) : (curTop = parseFloat(curCSSTop) || 0, curLeft = parseFloat(curCSSLeft) || 0),
        ...
        $.rails = rails = {
            ...
            disableFormElement: function (element) {
                ...
                (replacement = element.data("disable-with")) !== undefined && (element.data("ujs:enable-with", element[method]()), element[method](replacement)),
            ...
            enableFormElement: function (element) {
                ...
                element.data("ujs:enable-with") !== undefined && (element[method](element.data("ujs:enable-with")), element.removeData("ujs:enable-with")),
            ...
            blankInputs: function (form, specifiedSelector, nonBlank) {
                ...
                return requiredInputs.each(function () {
                    (input = $(this)).is("input[type=radio]") ? (radioName = input.attr("name"), checkedRadioButtonNames[radioName] || (0 === form.find('input[type=radio]:checked[name="' + radioName + '"]').length && (radiosForNameWithNoneSelected = form.find('input[type=radio][name="' + radioName + '"]'), foundInputs = foundInputs.add(radiosForNameWithNoneSelected)), checkedRadioButtonNames[radioName] = radioName)) : (input.is("input[type=checkbox],input[type=radio]") ? input.is(":checked") : !!input.val()) === nonBlank && (foundInputs = foundInputs.add(input))
                }),
            ...
            disableElement: function (element) {
                ...
                replacement !== undefined && (element.data("ujs:enable-with", element.html()), element.html(replacement)),
        ...
        rails.fire($document, "rails:attachBindings") && ($.ajaxPrefilter(function (options, originalOptions, xhr) {
                ...
                if (!rails.allowAction(link))
                    return rails.stopEverything(e);
                if (!metaClick && link.is(rails.linkDisableSelector) && rails.disableElement(link), rails.isRemote(link)) {
                ...
                var link = $(this);
                return rails.allowAction(link) && rails.isRemote(link) ? (rails.handleRemote(link), !1) : rails.stopEverything(e)
    ...
    Carousel.prototype.slide = function (type, next) {
        ...
        if (this.$element.trigger(slideEvent), !slideEvent.isDefaultPrevented()) {
            ...
            return $.support.transition && this.$element.hasClass("slide") ? ($next.addClass(type), $next[0].offsetWidth, $active.addClass(direction), $next.addClass(direction), $active.one("bsTransitionEnd", function () {
                ...
                }).emulateTransitionEnd(1e3 * $active.css("transition-duration").slice(0, -1))) : ($active.removeClass("active"), $next.addClass("active"), this.sliding = !1, this.$element.trigger(slidEvent)),
...
(jQuery), function ($) {
    ...
    $(document).on("click.bs.collapse.data-api", '[data-toggle="collapse"]', function (e) {
        ...
        data && data.transitioning || ($parent && $parent.find('[data-toggle="collapse"][data-parent="' + parent + '"]').not($this).addClass("collapsed"), $this[$target.hasClass("in") ? "addClass" : "removeClass"]("collapsed")),
        ...
        if (/(38|40|27)/.test(e.keyCode)) {
            ...
            if (e.preventDefault(), e.stopPropagation(), !$this.is(".disabled, :disabled")) {
                ...
                if (!isActive || isActive && 27 == e.keyCode)
                    return 27 == e.which && $parent.find(toggle).trigger("focus"), $this.trigger("click");
...
(jQuery), function ($) {
    ...
    Modal.prototype.show = function (_relatedTarget) {
        ...
        this.isShown || e.isDefaultPrevented() || (this.isShown = !0, this.checkScrollbar(), this.$body.addClass("modal-open"), this.setScrollbar(), this.escape(), this.$element.on("click.dismiss.bs.modal", '[data-dismiss="modal"]', $.proxy(this.hide, this)), this.backdrop(function () {
...
(jQuery), function ($) {
    ...
    Tooltip.prototype.enter = function (obj) {
        ...
        if (self || (self = new this.constructor(obj.currentTarget, this.getDelegateOptions()), $(obj.currentTarget).data("bs." + this.type, self)), clearTimeout(self.timeout), self.hoverState = "in", !self.options.delay || !self.options.delay.show)
            return self.show();
    ...
    Tooltip.prototype.leave = function (obj) {
        var self = obj instanceof this.constructor ? obj : $(obj.currentTarget).data("bs." + this.type);
        if (self || (self = new this.constructor(obj.currentTarget, this.getDelegateOptions()), $(obj.currentTarget).data("bs." + this.type, self)), clearTimeout(self.timeout), self.hoverState = "out", !self.options.delay || !self.options.delay.hide)
            return self.hide();
        self.timeout = setTimeout(function () {
    ...
    Tooltip.prototype.hide = function () {
        ...
        if (this.$element.removeAttr("aria-describedby"), this.$element.trigger(e), !e.isDefaultPrevented())
            return $tip.removeClass("in"), $.support.transition && this.$tip.hasClass("fade") ? $tip.one("bsTransitionEnd", complete).emulateTransitionEnd(150) : complete(), this.hoverState = null, this
    },
...
(jQuery), function ($) {
    ...
    ScrollSpy.prototype.process = function () {
        ...
        if (this.scrollHeight != scrollHeight && this.refresh(), maxScroll <= scrollTop)
    ...
    Affix.prototype.checkPosition = function () {
        if (this.$element.is(":visible")) {
            ...
            if (this.affixed !== affix) {
                ...
                this.$element.trigger(e),
                e.isDefaultPrevented() || (this.affixed = affix, this.unpin = "bottom" == affix ? this.getPinnedOffset() : null, this.$element.removeClass(Affix.RESET).addClass(affixType).trigger($.Event(affixType.replace("affix", "affixed"))), "bottom" == affix && this.$element.offset({
...
(function () {
    ...
    factory = function () {
        ...
        setListener = !(customEvent = {
                ...
                remove: function (obj, event, callback) {
                    ...
                    if (callback) {
                        if (obj[cgid] && listeners[obj[cgid]] && listeners[obj[cgid]][event])
                            for (i = j = 0, len = (ref = listeners[obj[cgid]][event]).length; j < len; i = ++j)
                                if ((cl = ref[i]) === callback)
                                    return listeners[obj[cgid]][event].splice(i, 1), cl
                ...
                fire: function (obj, event, memo) {
                    ...
                    results;
                    if (obj[cgid] && listeners[obj[cgid]] && listeners[obj[cgid]][event]) {
                        for (results = [], j = 0, len = (ref = listeners[obj[cgid]][event]).length; j < len; j++)
                            ev = ref[j], results.push(ev(memo || {}));
                        return results
                    }
                }
            }),
        ...
        function (element, event) {
            var evt;
            return doc.createEventObject ? element.fireEvent("on" + event, evt) : ((evt = doc.createEvent("HTMLEvents")).initEvent(event, !0, !0), !element.dispatchEvent(evt))
        },
        ie = function () {
            var all,
            check,
            ...
            undef,
            v;
            for (undef = void 0, v = 3, div = doc.createElement("div"), all = div.getElementsByTagName("i"), check = function () {
        ...
        init = function () {
            var blur;
            return !!initialized || (!1 === hidden ? (blur = "blur", ie < 9 && (blur = "focusout"), addEvent(window, blur, function () {
        ...
        ifvisible = {
            ...
            getIdleInfo: function () {
                ...
                return now = +new Date,
                res = {},
                "idle" === status ? (res.isIdle = !0, res.idleFor = now - idleStartedTime, res.timeLeft = 0, res.timeLeftPer = 100) : (res.isIdle = !1, res.idleFor = now - idleStartedTime, res.timeLeft = idleStartedTime + idleTime - now, res.timeLeftPer = (100 - 100 * res.timeLeft / idleTime).toFixed(2)),
                res
            },
            focus: function (callback) {
                return "function" == typeof callback ? this.on("focus", callback) : (status = "active", customEvent.fire(this, "focus"), customEvent.fire(this, "wakeup"), customEvent.fire(this, "statusChanged", {
                        status: status
                    })),
                this
            },
            blur: function (callback) {
                return "function" == typeof callback ? this.on("blur", callback) : (status = "hidden", customEvent.fire(this, "blur"), customEvent.fire(this, "idle"), customEvent.fire(this, "statusChanged", {
                        status: status
                    })),
                this
            },
            idle: function (callback) {
                return "function" == typeof callback ? this.on("idle", callback) : (status = "idle", customEvent.fire(this, "idle"), customEvent.fire(this, "statusChanged", {
                        status: status
                    })),
                this
            },
            wakeup: function (callback) {
                return "function" == typeof callback ? this.on("wakeup", callback) : (status = "active", customEvent.fire(this, "wakeup"), customEvent.fire(this, "statusChanged", {
                        status: status
                    })),
                this
            },
    ...
    ajaxRequest = function () {
        var activexmodes,
        i;
        if (activexmodes = ["Msxml2.XMLHTTP", "Microsoft.XMLHTTP"], !window.ActiveXObject)
    ...
    handleBackgroundImages = function () {
        return null != window.background_image_1440 && null != window.background_image_2880 ? 1440 < $(document).width() || 1440 < $(document).height() ? (console.log("F"), $("html").css("background-image", "url(" + window.background_image_2880 + ")")) : (console.log("a"), $("html").css("background-image", "url(" + window.background_image_1440 + ")")) : ($(".dumbBoxOverlay").css("display", "none"), $(".appBox").css("background-color", "transparent"))
    },
    handleAppImages = function () {
        var app;
        if (app = window.app.toLowerCase(), window.app_icon ? $("#app-download-img").attr("src", window.app_icon) : "android" === window.os_family.toLowerCase() ? ($("#app-download-img").attr("src", "/assets/google_play_store.png"), $("#app-download-img2").attr("src", "/assets/google_play_store.png")) : "ios" === window.os_family.toLowerCase() && ($("#app-download-img").attr("src", "/assets/app_store_icon.png"), $("#app-download-img2").attr("src", "/assets/app_store_icon.png")), "custom" === app)
            return $("#app-img").hide()
    },
    setDelays = function () {
        if (window.app_open_delay = 1e3, window.appstore_open_delay = 1e3, window.isMobile.iOS() && (window.delay_check = 100 + 1e3 * window.ios_delay, window.delay = 1e3 * window.ios_delay, window.app_open_delay = 1e3 * window.ios_delay, window.appstore_open_delay = 1e3 * window.ios_delay, window.total_delay = window.app_open_delay + window.app_store_open_delay, window.action_success = !1, window.app_open_delay < 250 && (window.app_open_delay = 250), window.appstore_open_delay < 250))
    ...
    handleClickEvents = function () {
        ...
        $("#browser-text").click(function (event) {
            return window.location.href.includes("disabled=true") || (event.preventDefault(), browser_go()),
            !1
        }),
        ...
        $("#app-text").click(function (event) {
            return window.location.href.includes("disabled=true") || (event.preventDefault(), setAppOpenGo()),
            !1
        })
    },
    ...
    runner = function () {
        ...
        -1 === window.location.href.indexOf("/android") && -1 === window.location.href.indexOf("/ios") ? ($("#app-button").click(function () {
                ...
                window.location = window.app_store_url
            })) : (console.log("Runner----"), window.app_store_url && console.log(window.app_store_url), window.app_scheme && console.log(window.app_scheme), console.log("Domain: " + window.domain), window.action = Cookies.get("urlgAction", {
                domain: window.domain
            }), console.log("Action: " + window.action), "openApp" === window.action && (Cookies.remove("urlgAction", {
                    domain: window.domain
                }), openApp()), "openAppstore" === window.action && (Cookies.remove("urlgAction", {
                    domain: window.domain
                }), document.hidden || openAppstore()), "clearAll" === window.action && (Cookies.remove("urlgAction", {
                    domain: window.domain
                }), document.hidden || saveFallbackUrlTry()), window.location.href.includes("app_opened=false") && updateHit("?app_opened=false"), handleClickEvents())
    },
    browser_go = function () {
        var hip,
        url;
